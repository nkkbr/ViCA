...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            sam_image = Image.open(image_file).convert('RGB')
            sam_image_array = np.array(sam_image)
            sam_image_array = np.expand_dims(sam_image_array, axis=0)

            sam_processor = self.data_args.image_processor_for_sam
            image_for_sam = sam_processor.preprocess(sam_image_array, return_tensors="pt")["pixel_values"]
            ...
            try:
                ...
                # only for vica2
                image_for_sam = sam_processor.preprocess(video, return_tensors="pt")["pixel_values"]
        ...
        if "image" in self.list_data_dict[i]:
            ...
            # only for vica2
            data_dict['image_for_sam']=image_for_sam
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # only for vica2
        if 'image_for_sam' in instances[0]:
            batch['images_for_sam'] = [instance['image_for_sam'] for instance in instances]
        else:
            batch['images_for_sam'] = [[] for _ in instances]
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        # only for vica2
        elif "vica" in model_args.model_name_or_path.lower():
            model = VicaQwenForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        sam_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        data_args.image_processor_for_sam = sam_tower.image_processor
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            # only for sam2
            sam_tower.requires_grad_(False)
            model.model.sam_projector.requires_grad_(False)
            ...
            # only for vica2
            if "sam_tower" in tunable_parts:
                for name, param in model.named_parameters():
                    if "sam_tower" in name:
                        param.requires_grad_(True)
            if "sam_projector" in tunable_parts:
                for name, param in model.named_parameters():
                    if "sam_projector" in name:
                        param.requires_grad_(True)
        # only for vica2
        # check trainable parameters
        for name, param in model.named_parameters():
            if param.requires_grad is True:
                rank0_print(f"trainable_params:    {name}")
